...
export const CheckoutView: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    } else if (!/^\d{10,12}$/.test(mobileNumber.replace(/[-\s]/g, ''))) {
      newErrors.mobileNumber = 'Please enter a valid mobile number (e.g. 03xxxxxxxx)';
    }
  ...
  if (itemCount === 0 && !orderSuccess) {
    return (
      <div className="max-w-md mx-auto py-16 px-6 text-center bg-white border border-gray-100 rounded-2xl shadow-sm mt-8 space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 max-w-[240px] leading-relaxed mx-auto mt-1">
            Add some delicious meals from Ghalib Restaurant before proceeding to checkout.
          </p>
        </div>
        <button
          onClick={() => setIsCheckoutActive(false)}
          className="px-5 py-2.5 bg-brand-primary text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-transform active:scale-95 cursor-pointer"
        >
          Go Back To Menu
        </button>
      </div>
  ...
  return (
    <div id="checkout-view-wrapper" className="w-full max-w-[1200px] mx-auto px-4 py-6 md:py-8 select-none">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="border border-gray-150 bg-white shadow-sm p-4 rounded-2xl flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-full bg-[#EAF2EE] border border-[#D2E2DC] flex items-center justify-center text-[#1A3C2E] shrink-0">
            <Bike className="w-6 h-6" strokeWidth={2} />
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs md:text-sm text-gray-600 font-medium">This is a</span>
              <span className="bg-[#052E16] text-white text-[10px] md:text-[11px] font-extrabold px-3 py-0.5 rounded-full uppercase tracking-wider">
                Delivery Order
              </span>
            </div>
      ...
      <form id="checkout-form" onSubmit={handleSubmitOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 mt-6">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {/* Form Fields Card */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6 space-y-4 md:space-y-5">
              ...
              <div className="md:col-span-1">
                ...
                <div className="relative">
                  <button
                    ...
                    className="h-11 bg-brand-primary hover:bg-brand-primary-hover border border-brand-primary/10 rounded-xl px-4 pr-10 w-full text-white text-sm font-extrabold flex items-center justify-between cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
                  >
                    <span>{title}</span>
                    <ChevronDown className={`text-white transition-transform duration-200 ${isTitleOpen ? 'rotate-180' : ''}`} size={16} strokeWidth={2.5} />
                  </button>
                  {isTitleOpen && (
                    <>
                      {/* Invisible backdrop overlay to handle clicks outside */}
                      <div className="fixed inset-0 z-40" onClick={() => setIsTitleOpen(false)} />
                      <div className="absolute top-12 left-0 right-0 bg-brand-primary border border-brand-primary/20 rounded-xl shadow-xl py-1 z-50 overflow-hidden">
                        {['Mr.', 'Ms.', 'Mrs.'].map((val) => (
                          <div
                            key={val}
                            onClick={() => {
                            ...
                            className={`px-4 py-2 text-sm font-bold text-white cursor-pointer transition-colors ${
                              title === val 
                                ? 'bg-[#0E2219] text-white font-black' 
                                : 'hover:bg-brand-primary-hover text-white/95'
                            }`}
                          >
                            {val}
                          </div>
            ...
            <div id="field-deliveryAddress" className="border border-gray-150 rounded-2xl p-4 bg-white space-y-3">
              ...
              <div className="bg-gray-50 border border-gray-100 rounded-lg p-2.5 flex items-center justify-between text-xs">
                <span className="text-gray-500 font-medium">Selected Neighborhood Area</span>
                <button
                  type="button"
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3.5">
              ...
              <div
                ...
                className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer select-none relative transition-all bg-white ${
                  paymentMethod === 'cod'
              ...
              >
                {paymentMethod === 'cod' && (
                  <div className="absolute top-2.5 right-2.5 w-4 h-4 bg-brand-primary text-white rounded-full flex items-center justify-center">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center">
              <div className="h-11 bg-gray-50 border border-r-0 border-gray-200 text-gray-500 text-xs font-bold px-4 flex items-center rounded-l-xl select-none">
                Rs.
              </div>
              <input
                type="text"
        ...
        <div ref={sectionTwoRef} className="lg:col-span-5 space-y-6">
          <div className="lg:sticky lg:top-24 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 md:p-5 space-y-4">
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    <div key={item.id} className="flex items-start gap-3">
                      ...
                      <div className="w-12 h-12 md:w-14 md:h-14 bg-gray-50 rounded-xl overflow-hidden shrink-0 border border-gray-100 relative">
                        {item.imageUrl ? (
                          <img
                            src={item.imageUrl}
                            alt={item.name}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center py-0.5">
                        <div className="flex items-baseline gap-1 flex-wrap">
                          <span className="text-xs md:text-[13px] font-black text-gray-900 shrink-0">
                            {item.quantity} x
                          </span>
                        ...
                        {item.variantSelections && item.variantSelections.length > 0 && (
                          <div className="mt-1 flex flex-wrap gap-1">
                            {item.variantSelections.map((sel) => (
                              <span 
                                key={sel.optionId} 
                                className="inline-block bg-gray-50 text-[9px] font-bold text-gray-500 px-1.5 py-0.5 rounded border border-gray-100 uppercase"
                              >
                                {sel.optionName}
                              </span>
                            ))}
              ...
              {/* Math breakdown rows */}
              <div className="bg-[#F8FBF9] border border-gray-100 rounded-2xl p-4 space-y-3 text-xs text-gray-600">
                
                {/* Total */}
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-2 font-medium">
                    <FileText size={14} className="text-gray-400" /> Total
                  </span>
                ...
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-2 font-medium">
                    <Percent size={14} className="text-gray-400" /> Tax ({config.taxPercent}%)
                  </span>
                  <span className="font-extrabold text-gray-800">
                    {formatPrice(tax)}
                  </span>
                ...
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-2 font-medium">
                    <Truck size={14} className="text-gray-400" /> Delivery Fee
                  </span>
                  <span className="font-extrabold text-gray-800">
                ...
                {/* Grand Total */}
                <div className="flex justify-between items-center text-gray-900 pt-0.5">
              ...
              <div ref={staticActionsRef} className="space-y-3.5 pt-1">
                <div>
                  <button
                    ...
                    className="w-full h-12 md:h-13 bg-[#052E16] hover:bg-[#032110] active:scale-[0.99] disabled:opacity-75 disabled:pointer-events-none text-white flex items-center justify-center gap-2 rounded-xl font-bold text-sm tracking-wide transition-all shadow-lg shadow-[#052E16]/10 cursor-pointer"
                  >
                    {isSubmitting ? (
                ...
                <div className="flex items-center justify-center">
                  <button
                    ...
                    className="text-xs font-bold text-gray-500 hover:text-gray-700 flex items-center gap-1 transition-all cursor-pointer hover:-translate-x-0.5 transform duration-150"
                  >
      ...
      <AnimatePresence>
        {!isStaticButtonVisible && isSectionTwoVisible && !orderSuccess && (
          <motion.div
            ...
            className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-150 py-3.5 px-4 shadow-[0_-8px_24px_rgba(0,0,0,0.06)] z-[90] flex flex-col items-center justify-center gap-2 lg:hidden"
          >
            <div className="w-full max-w-md flex flex-col sm:flex-row items-center gap-2.5">
              ...
              <button
                ...
                className="text-xs font-bold text-gray-500 hover:text-gray-700 flex items-center gap-1 transition-all cursor-pointer hover:-translate-x-0.5 transform duration-150 py-2 shrink-0 order-2 sm:order-1"
              >
                <span>&larr;</span> continue to add more items
              </button>
              ...
              <button
                ...
                form="checkout-form"
                disabled={isSubmitting}
                className="w-full sm:flex-1 h-11 bg-[#052E16] hover:bg-[#032110] active:scale-[0.99] disabled:opacity-75 disabled:pointer-events-none text-white flex items-center justify-center gap-2 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-lg shadow-[#052E16]/10 cursor-pointer order-1 sm:order-2"
              >
                {isSubmitting ? (
        ...
        {orderSuccess && (
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-[2rem] p-6 md:p-8 max-w-[480px] w-full shadow-2xl border border-gray-100 flex flex-col items-center select-none"
            >
              ...
              {/* Header Title */}
              <h2 className="text-xl md:text-2xl font-black text-[#0D1F18] text-center tracking-tight leading-snug">
                Order Placed Successfully! 🎉
              </h2>

              {/* Description */}
              <p className="text-center text-gray-500 text-xs sm:text-sm mt-2 leading-relaxed px-2">
                Thank you for ordering from Ghalib Restaurant. We are preparing your delicious meals right now.
              </p>

              {/* ID & Date Reference Row */}
              <div className="bg-[#F8FBF9] border border-gray-100 rounded-2xl p-4 w-full mt-5 space-y-2.5">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-500 font-medium flex items-center gap-1.5">
                    <FileText size={13} className="text-gray-400" /> Order ID
                  </span>
                  <span className="font-extrabold text-gray-900 tracking-wide uppercase">
                    {orderId}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-500 font-medium flex items-center gap-1.5">
                    <Clock size={13} className="text-gray-400" /> Delivery Estimated
                  </span>
                ...
                <div className="flex justify-between items-center text-xs">
                  <span className="text-gray-500 font-medium flex items-center gap-1.5">
                    <MapPin size={13} className="text-gray-400" /> Neighborhood Area
                  </span>
              ...
              <button
                ...
                className="w-full h-12 bg-[#052E16] hover:bg-[#032110] text-white flex items-center justify-center rounded-xl font-bold text-sm tracking-wide transition-all cursor-pointer shadow-lg mt-6"
              >
                Back To Menu
              </button>
