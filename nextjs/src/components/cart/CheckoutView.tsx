...
export const CheckoutView: React.FC = () => {
  ...
  const {
    ...
    altMobileNumber, setAltMobileNumber,
    deliveryAddress, setDeliveryAddress,
    nearestLandmark, setNearestLandmark,
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { setIsStaticButtonVisible(entry.isIntersecting); },
  ...
  useEffect(() => {
    ...
    if (!element) return;
    const observer = new IntersectionObserver(
      ([entry]) => { setIsSectionTwoVisible(entry.isIntersecting); },
      { root: null, threshold: 0 }
    );
    observer.observe(element);
    return () => { observer.unobserve(element); };
        ...
        <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div id="field-mobileNumber">
                ...
                <input
                  ...
                  value={mobileNumber}
                  onChange={(e) =>
                    setMobileNumber(formatPakistaniPhoneNumber(e.target.value))
                  }
              ...
              <div id="field-altMobileNumber">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setAltMobileNumber(
                      formatPakistaniPhoneNumber(e.target.value),
                    )
                  }
          ...
          <div className="space-y-3">
            ...
            {!config.paymentOptions || config.paymentOptions.length === 0 ? (
              <div className="p-4 border rounded-xl bg-red-50 text-red-600 text-sm">
                No payment options configured for this store.
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3.5">
                {config.paymentOptions.includes("cod") && (
                  <div
                ...
                {config.paymentOptions.includes("card") && (
                  <div
                    onClick={() => setPaymentMethod("card")}
                    className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer select-none relative transition-all bg-background-card ${paymentMethod === "card"
                      ? "border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary"
                ...
                {config.paymentOptions.includes("wallet") && (
                  <div
                    onClick={() => setPaymentMethod("wallet")}
                    className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer select-none relative transition-all bg-background-card ${paymentMethod === "wallet"
                      ? "border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary"
                      : "border-gray-200 hover:border-brand-primary/25"
                      }`}
                  >
                ...
                {config.paymentOptions.includes("bankTransfer") && (
                  <div
                    onClick={() => setPaymentMethod("bankTransfer")}
                    className={`border rounded-xl p-4 flex flex-col items-center justify-center gap-2 cursor-pointer select-none relative transition-all bg-background-card ${paymentMethod === "bankTransfer"
                      ? "border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary"
                      : "border-gray-200 hover:border-brand-primary/25"
                      }`}
                  >
